...
const TvShow = () => {
  const { TvShows, getSortedTvShows, tvGenre, getTvGenres } = useMoviesStore();
  const [visibleCount, setVisibleCount] = useState(5);
  const [selectedGenre, setSelectedGenre] = useState("");
  const [selectedSort, setSelectedSort] = useState("");

  const visibleMovies = TvShows.slice(0, visibleCount);
  useEffect(() => {
    getTvGenres();
    getSortedTvShows();
  }, []);
  ...
  useEffect(() => {
    getSortedTvShows(selectedGenre, selectedSort, true);
    setVisibleCount(5);
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      if (nearBottom) {
        ...
        if (visibleCount >= TvShows.length) {
          getSortedTvShows(selectedGenre, selectedSort);
        }
      }
    };
  ...
  }, [visibleCount, TvShows.length, selectedGenre, selectedSort]);
  return (
